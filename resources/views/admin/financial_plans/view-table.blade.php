...
<div class="table-custom-container position-relative  ">
    ...
    <x-tables.basic-view class="position-relative  main-table-class" id="{{ $tableId }}">
        <x-slot name="filter">
            @include('admin.financial_plans.filter' , [
            'type'=>'filter'
            ])
        </x-slot>

        <x-slot name="export">
            @include('admin.financial_plans.export' , [
            'type'=>'export'
            ])
        </x-slot>
...
//
                                      //              var select = `<select data-financial_plans-id="${row.id}" class="select select2-select change-duration-type">`;
...
//
                        ...
                        <a href="/` + $('body').data('lang') + '/' + $('body').data('current-company-id') + `/financial-plans/${row.id}/edit"  data-id="${row.id}" data-model-name="{{$modelName}}" class="btn btn-sm cursor-pointer btn-clean btn-icon btn-icon-md" title="{{ __('Edit Financial Plan Report') }}">
                        ...
                        `
                                        ...
                                        , initComplete() {
                                            ...
                                            $(document).find('.datepicker-input').datepicker({
                                                ...
                                                let financialPlanId = $(this).closest('tr').data('model-id')
                                                $(this).focus().html(dateString).blur();
                                                $.ajax({
                                                    url: "{{ route('admin.update.financial.plans.date',getCurrentCompanyId()) }}"
                                                    , data: {
                                                        "date": dateString
        ...
        <script>
            $(function() {
                $(document).on('change', '.change-duration-type', function() {
                    ...
                    if (durationType) {
                        $.ajax({
                            url: "{{ route('admin.update.financial.plan.duration.type',getCurrentCompanyId()) }}"
                            , data: {
                                durationType
                                , financialPlanId
                            }
