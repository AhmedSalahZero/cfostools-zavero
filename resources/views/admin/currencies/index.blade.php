...
@section('sub-header')
<x-main-form-title :id="'main-form-title'" :class="''">{{ __('Currencies') }}</x-main-form-title>
@endsection
    ...
    <div class="col-md-12">
        ...
        <div class="kt-portlet">
            ...
            <div class="kt-portlet__body" style="padding-bottom:0">
                ...
                <a href="{{ route('currencies.create',['company'=>$company->id]) }}" class="btn btn-bold btn-secondary  flex-1 flex-grow-0 btn-border-radius mr-auto">
            ...
            <x-table :tableClass="'kt_table_with_no_pagination_no_fixed  removeGlobalStyle ' ">
                ...
                <tr class="group-color main-row-tr">
                    ...
                    <td class="text-left text-capitalize"><b class="ml-3">
                            <span style="overflow: visible; position: relative; width: 110px;">
                                <a type="button" class="btn btn-secondary btn-outline-hover-brand btn-icon" title="Edit" href="{{ route('currencies.edit', ['company'=>$company->id , 'currency'=>$subItems['id']]) }}"><i class="fa fa-pen-alt"></i></a>
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h4 class="modal-title">{{ __('Delete Currency ' .$subItems['name']) }}</h4>
                                            </div>
                                            <div class="modal-body">
                                                <h3>{{ __('Are You Sure To Delete This Item ? ') }}</h3>
                                            </div>
                                            <form action="{{ route('currencies.destroy',['company'=>$company->id , 'currency'=> $subItems['id'] ]) }}" method="post" id="delete_form">
                                                {{ csrf_field() }}
                                                {{ method_field('DELETE') }}
                ...
                <tr class="row{{ $id }}  text-center sub-item-row" style="display: none">
                    ...
                    <td class="text-left text-capitalize"><b class="ml-3">
                            <span style="overflow: visible; position: relative; width: 110px;">
                                <a type="button" class="btn btn-secondary btn-outline-hover-brand btn-icon" title="Edit" href="{{ route('currencies.edit', ['company'=>$company->id , 'currency'=>$subItemArr['id']]) }}"><i class="fa fa-pen-alt"></i></a>
                                ...
                                <div id="modal-delete-{{ $subItemArr['id'] }}" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel1" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h4 class="modal-title">{{ __('Delete Currency ' .$subItemArr['name']) }}</h4>
                                            </div>
                                            <div class="modal-body">
                                                <h3>{{ __('Are You Sure To Delete This Item ? ') }}</h3>
                                            </div>
                                            <form action="{{ route('currencies.destroy',['company'=>$company->id , 'currency'=> $subItemArr['id'] ]) }}" method="post" id="delete_form">
                                                {{ csrf_field() }}
                                                {{ method_field('DELETE') }}
                                                <div class="modal-footer">
                                                    <button class="btn btn-danger">
