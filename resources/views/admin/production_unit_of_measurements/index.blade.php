...
@section('sub-header')
<x-main-form-title :id="'main-form-title'" :class="''">{{ __('Production Unit Of Measurements') }}</x-main-form-title>
@endsection
    ...
    <div class="col-md-12">
        ...
        <div class="kt-portlet">
            ...
            <div class="kt-portlet__body" style="padding-bottom:0">
                ...
                <a href="{{ route('production-unit-of-measurements.create',['company'=>$company->id]) }}" class="btn btn-bold btn-secondary  flex-1 flex-grow-0 btn-border-radius mr-auto">
            ...
            <x-table :tableClass="'kt_table_with_no_pagination_no_fixed  removeGlobalStyle ' ">
                ...
                <tr class="group-color main-row-tr">
                    <td class="black-text max-w-80" style="cursor: pointer;" onclick="toggleRow('{{ $id }}')">
                        ...
                        <div class="d-flex align-items-center ">
                            ...
                            @endif --}}
                            <b class="text-capitalize ">{{ $subItems['name'] }}</b>
                        </div>
                    </td>
                    ...
                    <td class="text-left text-capitalize"><b class="ml-3">
                            <span style="overflow: visible; position: relative; width: 110px;">
                                <a type="button" class="btn btn-secondary btn-outline-hover-brand btn-icon" title="Edit" href="{{ route('production-unit-of-measurements.edit', ['company'=>$company->id , 'production_unit_of_measurement'=>$subItems['id']]) }}"><i class="fa fa-pen-alt"></i></a>
                                <a class="btn btn-secondary btn-outline-hover-danger btn-icon  " href="#" data-toggle="modal" data-target="#modal-delete-{{ $subItems['id']}}" title="Delete"><i class="fa fa-trash-alt"></i>
                                </a>

                                <div id="modal-delete-{{ $subItems['id'] }}" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel1" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h4 class="modal-title">{{ __('Delete Production Unit Of Measurement ' .$subItems['name']) }}</h4>
                                            </div>
                                            <div class="modal-body">
                                                <h3>{{ __('Are You Sure To Delete This Item ? ') }}</h3>
                                            </div>
                                            <form action="{{ route('production-unit-of-measurements.destroy',['company'=>$company->id , 'production_unit_of_measurement'=> $subItems['id'] ]) }}" method="post" id="delete_form">
                                                {{ csrf_field() }}
                                                {{ method_field('DELETE') }}
                ...
                <tr class="row{{ $id }}  text-center sub-item-row" style="display: none">
                    ...
                    <td class="text-left text-capitalize"><b class="ml-3">
                            <span style="overflow: visible; position: relative; width: 110px;">
                                <a type="button" class="btn btn-secondary btn-outline-hover-brand btn-icon" title="Edit" href="{{ route('categories.edit', ['company'=>$company->id , 'category'=>$subItemArr['id']]) }}"><i class="fa fa-pen-alt"></i></a>
                                ...
                                <div id="modal-delete-{{ $subItemArr['id'] }}" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel1" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h4 class="modal-title">{{ __('Delete Category ' .$subItemArr['name']) }}</h4>
                                            </div>
                                            <div class="modal-body">
                                                <h3>{{ __('Are You Sure To Delete This Item ? ') }}</h3>
                                            </div>
                                            <form action="{{ route('categories.destroy',['company'=>$company->id , 'category'=> $subItemArr['id'] ]) }}" method="post" id="delete_form">
                                                {{ csrf_field() }}
                                                {{ method_field('DELETE') }}
                                                <div class="modal-footer">
                                                    <button class="btn btn-danger">
