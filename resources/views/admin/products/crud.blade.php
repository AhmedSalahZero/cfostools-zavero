...
@endsection
@section('sub-header')
<x-main-form-title :id="'main-form-title'" :class="''">{{ __('Products') }}</x-main-form-title>
@endsection
@section('content')
<div class="row">
    <div class="col-md-12">
                        ...
                        <div class="row">
                            <div @if(!isset($model)) id="m_repeater_2" @endif class="w-100">
                                <div class="form-group  m-form__group row">
                                    <div @if(!isset($model)) data-repeater-list="products" @endif class="col-lg-12">
                                        <div data-repeater-item class="form-group m-form__group row align-items-center repeater_item">
                                            ...
                                            <div class="col-md-6">
                                                <label>{{__('Category')}}<span class="astric">*</span></label>
                                                <div class="m-form__group m-form__group--inline">
                                                    <div class="m-form__control">
                                                        <select name="category_id" class="form-control">
                                                            @foreach( $categories as $category )
                                                            <option value="{{$category->id  }}" @if(isset($model )? $category->id ==$model->category_id :false ) selected @endif >{{$category->getName()}}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                </div>
