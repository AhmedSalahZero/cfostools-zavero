...
<div class="col-md-12 {{ $parentClass }}  js-parent-to-table" style="display:none">
    <hr style="width:100%;">
    <table id="{{ $repeaterId }}" class="table {{ $repeaterId }} table-white {{ $repeaterClass }} repeater-class repeater {{ $tableName }}"  >
        <thead>
            <tr>
                <x-tables.repeater-table-th class="col-md-1 {{ $actionThClass }}" :title="__('Action')"></x-tables.repeater-table-th>
                {{ $ths }}
        ...
        <td class="text-center">
			{{-- @if(!$isRepeater) --}}
            <div  data-repeater-create="" class="btn btn btn-sm text-white text-center  border-green bg-green  m-btn m-btn--icon m-btn--pill m-btn--wide {{__('right')}}" id="add-row">
                <span>
