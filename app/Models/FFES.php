...
class  FFES extends Model 
{

	use   CompanyScope;
	
	protected $table ='ffes';
	...
	public function getTotalItemsCost():float 
	{
		...
		$total = 0;
		$this->ffesItems->each(function($ffeItem) use (&$total){
	...
	public function getCollectionPolicyValue():array 
	{
		...
		public function ffesItems():HasMany
		{
		return $this->hasMany(FFESItem::class , 'ffe_id','id');
	}		
	...
	public function calculateFFEAssetsForFFE(string  $transferredDateForFFEAsString,float  $transferredAmount,array $studyDates,string $studyEndDate):array 
	{
		...
		$this->ffesItems->each(function(FFEItem $ffeItem) use ($totalItemsCost,$transferredDateForFFEAsString,$transferredAmount,&$assets,$studyDates,$studyEndDate){
	...
	/* start for zavero (financoal plan) */
	
	public function getBalanceRateThree()
	{
		return $this->balance_rate_three?:0;
	}
	
	public function getDueThree()
	{
		return $this->due_three?:0;
	}
	public function getOtherExpensesRate()
	{
		return $this->other_expenses_rate ?: 0;
	}
	public function getClearanceRate()
	{
		return $this->clearance_rate ?: 0;
	}
	public function getCustomRate()
	{
		return $this->custom_rate ?: 0;
	}
	public function getFreightRate()
	{
		return $this->freight_rate ?: 0;
	}
	public function getVatRate()
	{
		return $this->vat_rate ?: 0;
	}
	public function getCreditWithholdTaxRate()
	{
		return $this->credit_withhold_tax_rate ?: 0;
	}
	
	public function ffesItemsFor(string $sectionName,string $modelName)
	{
		return $this->ffesItems()->where('section_name',$sectionName)
		->where('model_name',$modelName)
		;
	}
	
	public function getGrantFunding()
	{
		return $this->ffe_grant_funding ?:0;
	}
	public function getGrantAmount()
	{
		$grantAmount = $this->getGrantFunding() / 100 ;
		return $grantAmount * $this->getTotalItemsCost();
	}
	
	/* end for zavero (financoal plan) */
